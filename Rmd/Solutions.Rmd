...
This document summarizes a collection of solutions for extracting recovery measures information from Recovery Documents using Reproducible Analytical Pipelines. Finddings will help identify best practices for future data entry and reporting. Outcomes of this project will include: 
...
Option1 <- DiagrammeR::grViz("digraph {
  ...
  node [shape = rectangle]        
  rec1 [label = 'Table in a PDF of selected Recovery Documents']
  ...
  height = 500)
```
Exploring solutions if tables in PDFs are the preffered way to store information
```{r workflow plot2}
Option1
```
...
Package ‘tabulizer’ was removed from the CRAN repository thus installation was conducted directly from GitHub. This leaves us at the mercy of package developers that could discotninue its mantainance. 
...
The output is a bit convoluted with a series of empty spaces and disjointed/unconnected text. Thus next step is to turn the table extraction into a dataframe to get an output that resembles the original table a tiny bit more, yet ussues prevailed and a bit more tweaking is required (coding side and/or way tabales are designed = the simpler the table, the eaiser for the code to extract it).  
...
The solution below still requires a bit more development but the simple extraction seems to be more similar to the original table 4. Keeping tables as simple as possible in the word or PDF file is one of the main pieces to implement moving forward. 
